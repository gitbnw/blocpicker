# Change these
server '104.131.169.224', port: 22, roles: [:web, :app, :db], primary: true
...
namespace :deploy do
  ...
  task :export do
     on roles (:app) do
      ...
      run "cd #{release_path} && rvmsudo bundle exec foreman export upstart /etc/init -a #{fetch(:application)} -u deploy -l #{shared_path}/log  -f #{release_path}/Procfile -c worker=1,scheduler=1, rweb=1"
    end 
  end
